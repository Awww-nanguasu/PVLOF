"""Apply PVLOF v1.5 confirmed-anomaly memory to a scored Parquet file."""
...
from pv_anomaly.pvlof_v15 import apply_confirmed_anomaly_memory, load_memory_config
...
def main() -> None:
    ...
    result = apply_confirmed_anomaly_memory(source, config)
    ...
    for plant, group in result.groupby("plant_id", observed=True):
        per_plant[str(plant)] = {
            "rows": len(group),
            "raw_anomaly_points": int(group["pvlof_v15_raw_anomaly"].sum()),
            "memory_reactivated_points": int(
                group["pvlof_v15_memory_reactivated_alert"].sum()
            ),
            "v1_5_alert_points": int(group["pvlof_v15_alert"].sum()),
        }
    report = {
        "input": args.input,
        "memory_config": args.memory_config,
        "rows": len(result),
        "raw_anomaly_points": int(result["pvlof_v15_raw_anomaly"].sum()),
        "memory_active_rows": int(result["pvlof_v15_memory_active"].sum()),
        "memory_reactivated_points": int(
            result["pvlof_v15_memory_reactivated_alert"].sum()
        ),
        "v1_5_alert_points": int(result["pvlof_v15_alert"].sum()),
        "events": len(events),
