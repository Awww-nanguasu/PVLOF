"""Apply PVLOF V1.7 Improved V4 to existing 58-day point files.

V4 keeps the V3 reference-member candidates and combines V1.6 and V3 raw
evidence before the three-point temporal confirmation. The replay reuses each
plant's frozen ``pvlof_v17_full_points.parquet`` and does not recalibrate.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default="configs/pvlof_v17_improved_v4_combined_candidate_memory.json",
    )
    ...
    if config.algorithm_variant != "combined_candidate_memory_v4":
        raise ValueError(
            "--config must select algorithm_variant=combined_candidate_memory_v4"
        )
    ...
    for directory in plant_directories:
        ...
        if existing and not args.overwrite:
            raise FileExistsError(
                f"V4 outputs already exist for plant_id={plant_id}: {existing}. "
                "Use --overwrite to replace only these known V4 files."
            )
    ...
    for directory in plant_directories:
        ...
        evidence, events, unconfirmed = _reconstruct_variant_events(
            ...
            segmentation_branch="improved_v4_combined_candidate_memory",
        )
        ...
        report.update({
            ...
            "temporal_confirmation": "combined_v16_or_v3_raw_by_string",
        })
